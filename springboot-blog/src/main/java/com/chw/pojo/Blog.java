...
public class Blog {
    ...
    //1,2,3
    private String tagsToIds(List<Tag> tags) {//拿到tags集合再处理集合
        if (!tags.isEmpty()) {
            StringBuffer ids = new StringBuffer();//定义ids
            boolean flag = false;
            for (Tag tag : tags) {//如果tag是flase,加后缀，给个，
                if (flag) {
                    ids.append(",");
                } else {
                    flag = true;
